...
<template>
<v-data-table :hide-default-footer="credentialsWithIndex.length < 10" v-model="selected" :show-select="selectable" single-select :headers="headers" :items="credentialsWithIndex" :expanded.sync="expanded" item-key="index" :show-expand="expandable" @click:row="openPresentation">
...
export default {
    ...
    methods: {
        deletePresentation(presentation) {
            ...
            this.$axios
                .delete(`${this.$apiBaseUrl}/partners/${partnerId}/proof/${presentation.id}`)
                .then(result => {
        ...
        openPresentation(presentation) {
            if (presentation.state === 'verified' || presentation.state == 'presentation_acked') {
