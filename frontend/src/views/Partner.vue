...
<v-container>
    <v-card class="mx-auto">
        <v-card-title class="bg-light">
            ...
            <v-text-field class="mt-8" v-else label="Name" append-icon="mdi-done" v-model="alias" outlined :rules="[rules.required]" dense>
                <template v-slot:append>
                    <v-btn class="pb-1" text @click="isUpdatingName = false">Cancel</v-btn>
                    <v-btn class="pb-1" text color="primary" :loading="isBusy" @click="submitNameUpdate()">Save</v-btn>
                </template>
            </v-text-field>
        ...
        <v-card-text>
            <OrganizationalProfile v-if="partner.profile" v-bind:documentData="partner.profile" isReadOnly></OrganizationalProfile>
            ...
            <v-row class="mx-4">
                <v-col cols="4">
                    ...
                    <v-row class="mt-4">
                        <v-btn small @click="requestPresentation">Request Presentation</v-btn>
                    </v-row>
                </v-col>
                <v-col cols="8">
                    <v-card flat>
                        <PresentationList v-if="isReady" v-bind:credentials="presentationsReceived" v-on:removedItem="removePresentationReceived" :expandable="false"></PresentationList>
                    </v-card>
                </v-col>
            </v-row>
            <v-row class="mx-4">
            ...
            <v-row class="mx-4">
                ...
                <v-col cols="8">
                    <PresentationList v-if="isReady" v-bind:credentials="presentationsSent" v-bind:headers="headersSent" v-on:removedItem="removePresentationSent" :expandable="false"></PresentationList>
                </v-col>
            </v-row>
        </v-card-text>
        ...
        <v-card-actions>
            <v-expansion-panels v-if="expertMode" accordion flat>
                <v-expansion-panel>
                    <v-expansion-panel-header class="grey--text text--darken-2 font-weight-medium bg-light">Show raw data</v-expansion-panel-header>
    ...
    <v-dialog v-model="attentionPartnerStateDialog" max-width="500">
        <v-card>
            <v-card-title class="headline">Connection State {{ partner.state }}
            </v-card-title>
