...
<template>
  <v-container>
    <v-card class="mx-auto">
      ...
      <v-card-text>
        <h4 v-if="credDef && credDef.fields && credDef.fields.length">Credential Content</h4>
        <v-row>
          <v-col>
            <v-text-field
  ...
  export default {
    ...
    methods: {
      async load() {
        ...
        if (presp.status === 200) {
          this.partners = presp.data.map(p => { return {value: p.id, text: p.alias, ...p}; });
          if (this.$props.partnerId) {
            this.partner = this.partners.find(p => p.value === this.$props.partnerId);
          }
        }
